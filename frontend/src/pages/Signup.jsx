...
export default function Signup() {
    ...
    const handleCompleteProfile = async (e) => {
        ...
        localStorage.setItem('genx_current_user', JSON.stringify(updatedUser));
                ...
                <div className="auth-card">
                    ...
                    {step === 1 && (
                        <form className="auth-form" onSubmit={handleCreateAccount}>
                            ...
                            <button className="btn btn-primary" type="submit">Next: Set Up Profile <ArrowRight size={16} /></button>
                        </form>
                    )}
                        ...
                        <form className="auth-form" onSubmit={handleCompleteProfile}>
                            ...
                            <div className="profile-photo-upload" onClick={() => fileInputRef.current?.click()}>
                                {profilePhotoPreview ? (
                                    ...
                                    <div className="profile-photo-placeholder"><Camera size={28} /><span>Add Photo</span></div>
                                )}
                                <div className="profile-photo-badge"><Upload size={14} /></div>
                            ...
                            <div className="input-with-icon">
                                ...
                                <div className="auth-phone-group" style={{ flex: 1 }}>
                                    <div className="auth-country-code" style={{ padding: '10px 10px' }}><span>🇮🇳</span><span>+91</span></div>
                                    <input className="input-field" type="tel" name="phone" placeholder="Phone number *" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value.replace(/\D/g, '').slice(0, 10) })} maxLength={10} />
                                </div>
                            </div>
