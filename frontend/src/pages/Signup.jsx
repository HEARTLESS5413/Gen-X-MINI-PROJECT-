...
import { ArrowRight, Camera, Check, Upload, User, Phone as PhoneIcon, AtSign, Image } from 'lucide-react';
...
export default function Signup() {
    ...
    const clerkSignUp = CLERK_KEY ? useSignUp() : null;
    const signUp = clerkSignUp?.signUp;
    const isLoaded = clerkSignUp?.isLoaded ?? true;

    const [step, setStep] = useState(1); // 1: google/create, 2: profile setup
    ...
    const handleGoogleSignup = async () => {
        if (!signUp || !isLoaded) {
            setError('Google sign-up is not available. Use the form below.');
            return;
        }
        setLoading(true);
        setError('');
        try {
            await signUp.authenticateWithRedirect({
                ...
                redirectUrlComplete: '/signup?step=profile',
            });
        } catch (err) {
